...
func TestMultiSenderRespectsLevel(t *testing.T) {
	...
	multi.Send(t.Context(), message.NewDefaultMessage(level.Info, "hello"))
	assert.Equal(mock.Len(), 1)
	m, ok := mock.GetMessageSafe()
	assert.True(ok)
	assert.False(m.Logged)

	multi.Send(t.Context(), message.NewDefaultMessage(level.Alert, "hello"))
	assert.Equal(mock.Len(), 1)
	m, ok = mock.GetMessageSafe()
	assert.True(ok)
	...
	assert.NoError(multi.Flush(t.Context()))
}

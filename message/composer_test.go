...
func TestPopulatedMessageComposerConstructors(t *testing.T) {
	...
	cases := map[Composer]string{
		...
		NewExtendedString(testMsg):                                              testMsg,
		NewExtendedDefaultMessage(level.Error, testMsg):                         testMsg,
		NewBytes([]byte(testMsg)):                                               testMsg,
		NewBytesMessage(level.Error, []byte(testMsg)):                           testMsg,
		NewExtendedBytes([]byte(testMsg)):                                       testMsg,
		NewExtendedBytesMessage(level.Error, []byte(testMsg)):                   testMsg,
		NewError(errors.New(testMsg)):                                           testMsg,
		NewErrorMessage(level.Error, errors.New(testMsg)):                       testMsg,
		NewExtendedError(errors.New(testMsg)):                                   testMsg,
		NewExtendedErrorMessage(level.Error, errors.New(testMsg)):               testMsg,
		...
		NewExtendedFieldsMessage(level.Error, testMsg, Fields{}):                fmt.Sprintf("[message='%s']", testMsg),
		NewExtendedFields(level.Error, Fields{"test": testMsg}):                 fmt.Sprintf("[test='%s']", testMsg),
		MakeExtendedFieldsMessage(testMsg, Fields{}):                            fmt.Sprintf("[message='%s']", testMsg),
		MakeExtendedFields(Fields{"test": testMsg}):                             fmt.Sprintf("[test='%s']", testMsg),
